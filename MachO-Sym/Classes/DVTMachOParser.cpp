...
#pragma mark - DVTMachOParser

DVTMachOParser::DVTMachOParser(const mach_header* mh, bool dyldCacheIsRaw)
{
#if DYLD_IN_PROCESS
...
const mach_header* DVTMachOParser::header() const
{
...
bool DVTMachOParser::isRaw() const
{
    return (_data & 1);
}
...
bool DVTMachOParser::inRawCache() const
{
    return (_data & 2);
}

uint32_t DVTMachOParser::fileType() const
{
    return header()->filetype;
}

bool DVTMachOParser::inDyldCache() const
{
    return (header()->flags & 0x80000000);
}

bool DVTMachOParser::hasThreadLocalVariables() const
{
    return (header()->flags & MH_HAS_TLV_DESCRIPTORS);
}

Platform DVTMachOParser::platform() const
{
    Platform platform;
    uint32_t minOS;
    uint32_t sdk;
...
const DVTMachOParser::ArchInfo DVTMachOParser::_s_archInfos[] = {
...
bool DVTMachOParser::isMachO(Diagnostics& diag, const void* fileContent, size_t mappedLength)
{
    // sanity check length
...
bool DVTMachOParser::wellFormedMachHeaderAndLoadCommands(const mach_header* mh)
{
...
//Platform DVTMachOParser::currentPlatform()
//{
//#if TARGET_OS_BRIDGE
...
void DVTMachOParser::forEachLoadCommand(Diagnostics& diag, void (^callback)(const load_command* cmd, bool& stop)) const
{
    bool stop = false;
    const load_command* startCmds = nullptr;
...
uint64_t DVTMachOParser::preferredLoadAddress() const
{
...
bool DVTMachOParser::getPlatformAndVersion(Platform* platform, uint32_t* minOS, uint32_t* sdk) const
{
    Diagnostics diag;
...
bool DVTMachOParser::isSimulatorBinary() const
{
    Platform platform;
    uint32_t minOS;
    uint32_t sdk;
...
bool DVTMachOParser::getDylibInstallName(const char** installName, uint32_t* compatVersion, uint32_t* currentVersion) const
{
    Diagnostics diag;
    __block bool found = false;
    forEachLoadCommand(diag, ^(const load_command* cmd, bool& stop) {
...
const char* DVTMachOParser::installName() const
{
...
uint32_t DVTMachOParser::dependentDylibCount() const
{
...
const char* DVTMachOParser::dependentDylibLoadPath(uint32_t depIndex) const
{
...
void DVTMachOParser::forEachDependentDylib(void (^callback)(const char* loadPath, bool isWeak, bool isReExport, bool isUpward, uint32_t compatVersion, uint32_t curVersion, bool& stop)) const
{
    Diagnostics diag;
    forEachLoadCommand(diag, ^(const load_command* cmd, bool& stop) {
        switch ( cmd->cmd ) {
...
void DVTMachOParser::forEachRPath(void (^callback)(const char* rPath, bool& stop)) const
{
    Diagnostics diag;
    forEachLoadCommand(diag, ^(const load_command* cmd, bool& stop) {
        if ( cmd->cmd == LC_RPATH ) {
...
const uint8_t* DVTMachOParser::getLinkEditContent(const LayoutInfo& info, uint32_t fileOffset) const
{
#if DYLD_IN_PROCESS
...
void DVTMachOParser::getLayoutInfo(LayoutInfo& result) const
{
#if DYLD_IN_PROCESS
...
void DVTMachOParser::forEachSection(void (^callback)(const char* segName, const char* sectionName, uint32_t flags,
...
void DVTMachOParser::forEachSection(void (^callback)(const char* segName, const char* sectionName, uint32_t flags, uint64_t addr,
...
void DVTMachOParser::forEachSection(void (^callback)(const char* segName, uint32_t segIndex, uint64_t segVMAddr, const char* sectionName, uint32_t sectFlags,
...
void DVTMachOParser::forEachGlobalSymbol(Diagnostics& diag, void (^callback)(const char* symbolName, uint64_t n_value, uint8_t n_type, uint8_t n_sect, uint16_t n_desc, bool& stop)) const
{
    LinkEditInfo leInfo;
...
void DVTMachOParser::forEachLocalSymbol(Diagnostics& diag, void (^callback)(const char* symbolName, uint64_t n_value, uint8_t n_type, uint8_t n_sect, uint16_t n_desc, bool& stop)) const
{
    LinkEditInfo leInfo;
    getLinkEditPointers(diag, leInfo);
    if ( diag.hasError() )
...
bool DVTMachOParser::findExportedSymbol(Diagnostics& diag, const char* symbolName, void* extra, FoundSymbol& foundInfo, DependentFinder findDependent) const
{
    LinkEditInfo leInfo;
    getLinkEditPointers(diag, leInfo);
    if ( diag.hasError() )
    ...
    if ( leInfo.dyldInfo != nullptr ) {
            ...
            forEachDependentDylib(^(const char* loadPath, bool isWeak, bool isReExport, bool isUpward, uint32_t compatVersion, uint32_t curVersion, bool& stop) {
                if ( isReExport && findDependent ) {
                    ...
                    if ( findDependent(depIndex, loadPath, extra, &depMH, &depExtra) ) {
                        ...
                        DVTMachOParser dep(depMH, depInRawCache);
        ...
        if ( flags & EXPORT_SYMBOL_FLAGS_REEXPORT ) {
            ...
            if ( findDependent(depIndex, dependentDylibLoadPath(depIndex), extra, &depMH, &depExtra) ) {
                bool depInRawCache = inRawCache() && (depMH->flags & 0x80000000);
                DVTMachOParser depParser(depMH, depInRawCache);
...
void DVTMachOParser::getLinkEditLoadCommands(Diagnostics& diag, LinkEditInfo& result) const
{
...
void DVTMachOParser::getLinkEditPointers(Diagnostics& diag, LinkEditInfo& result) const
{
...
void DVTMachOParser::forEachSegment(void (^callback)(const char* segName, uint32_t fileOffset, uint32_t fileSize, uint64_t vmAddr, uint64_t vmSize, uint8_t protections, bool& stop)) const
{
    Diagnostics diag;
    forEachLoadCommand(diag, ^(const load_command* cmd, bool& stop) {
        if ( cmd->cmd == LC_SEGMENT_64 ) {
...
const uint8_t* DVTMachOParser::trieWalk(Diagnostics& diag, const uint8_t* start, const uint8_t* end, const char* symbol)
{
...
uint64_t DVTMachOParser::read_uleb128(Diagnostics& diag, const uint8_t*& p, const uint8_t* end)
{
    uint64_t result = 0;
    int         bit = 0;
    do {
...
int64_t DVTMachOParser::read_sleb128(Diagnostics& diag, const uint8_t*& p, const uint8_t* end)
{
    int64_t  result = 0;
    int      bit = 0;
    uint8_t  byte = 0;
...
bool DVTMachOParser::is64() const
{
#if DYLD_IN_PROCESS
...
bool DVTMachOParser::findClosestSymbol(uint64_t targetUnslidAddress, const char** symbolName, uint64_t* symbolUnslidAddr) const
{
    Diagnostics diag;
...
bool DVTMachOParser::findClosestSymbol(const void* addr, const char** symbolName, const void** symbolAddress) const
{
...
intptr_t DVTMachOParser::getSlide() const
{
    Diagnostics diag;
...
bool DVTMachOParser::hasExportedSymbol(const char* symbolName, DependentFinder finder, void** result) const
{
...
const char* DVTMachOParser::segmentName(uint32_t targetSegIndex) const
{
...
bool DVTMachOParser::isFairPlayEncrypted(uint32_t& textOffset, uint32_t& size)
{
    textOffset = 0;
    size = 0;
    Diagnostics diag;
